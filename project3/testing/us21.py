...
def verify_correct_roles(indivs, fams):
    for fam_id in fams:
        ...
        if indivs[husb_id][2] != 'M':
            return (False, "ERROR: INDIVIDUAL: US21: {}: Incorrect role for husband in family {}".format(husb_id, fam_id))
        if indivs[wife_id][2] != 'F':
            return (False, "ERROR: INDIVIDUAL: US21: {}: Incorrect role for wife in family {}".format(wife_id, fam_id))
    return (True, "")
